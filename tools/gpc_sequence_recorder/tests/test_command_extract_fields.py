...
def test_schema_includes_var_mul_and_var_add():
    ...
    assert "var_bytes_assign" in schema.micro_ops


def test_var_bytes_assign_records_and_emits():
    engine = ReplEngine(auto_reload=False)
    lines = [
        "bind_main_tick()",
        "var_set(var_index=1, value=[1, 2, 3, 0])",
        "var_set(var_index=2, value=0x104)",
        "var_bytes_assign(dest_var_index=1, byte_index=3, byte_count=1, src_var_index=2)",
        "end_binding()",
    ]
    for line in lines:
        out, cont = engine.execute(line)
    ...
    assert steps[2].union_member == "var_bytes_assign"
    assert steps[2].values["dest_var_index"] == 1
    assert steps[2].values["byte_index"] == 3
    assert steps[2].values["byte_count"] == 1
    assert steps[2].values["src_var_index"] == 2

    text = emit_config_hpp(engine.ctx.session.to_dict(), engine.ctx.schema, write=False)
    assert "MicroOpType::VAR_BYTES_ASSIGN" in text
    assert ".var_bytes_assign" in text


def test_var_bytes_assign_rejects_invalid_byte_index():
    engine = ReplEngine(auto_reload=False)
    out, cont = engine.execute("bind_main_tick()")
    assert cont, out
    out, cont = engine.execute(
        "var_bytes_assign(dest_var_index=1, byte_index=8, byte_count=1, src_var_index=2)"
    )
